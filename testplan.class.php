...
class testplan extends tlObjectWithAttachments
{
  ...
  var $nt2exclude = array(
    'testplan' => 'exclude_me',
    'requirement_spec' => 'exclude_me',
    'requirement' => 'exclude_me'
  );

  var $nt2exclude_children = array(
  ...
  function unlink_tcversions($id, &$items)
  {
    ...
    foreach ($items['items'] as $tcase_id => $elem) {
      foreach ($elem as $platform_id => $tcversion) {
        ...
        $auditMsg = TLS(
          "audit_tc_removed_from_testplan",
          $auditInfo['tcasePrefix'] . $auditInfo['info'][$tcversion]['tc_external_id'] .
          $title_separator . $auditInfo['info'][$tcversion]['name'],
          $auditInfo['info'][$tcversion]['version'],
          $auditInfo['tplanInfo']['name'] . $addInfo
  ...
  function addUserRole($userID, $id, $roleID)
  {
    ...
    if ($this->db->exec_query($sql)) {
      ...
      if ($user && $testPlan && $role) {
        logAuditEvent(TLS(
          "audit_users_roles_added_testplan",
          $user->getDisplayName(),
          $testPlan['name'],
          $role->name
        ), "ASSIGN", $id, "testplans");
  ...
  function deleteUserRoles($id, $users = null, $opt = null)
  {
    ...
    if ($this->db->exec_query($sql) && $my['opt']['auditlog']) {
      ...
      if ($testPlan) {
        if (is_null($users)) {
          logAuditEvent(TLS(
            "audit_all_user_roles_removed_testplan",
            $testPlan['name']
          ), "ASSIGN", $id, "testplans");
        } else {
          // TBD
  ...
  */
  function get_linked_cfields_at_design($id, $parent_id = null, $show_on_execution = null)
  {
    ...
    $cf_map = $this->cfield_mgr->get_linked_cfields_at_design(
      $tproject_id,
      self::ENABLED,
      $show_on_execution,
      'testplan',
  ...
  */
  function get_linked_cfields_at_execution($id, $parent_id = null, $show_on_execution = null)
  {
    ...
    $cf_map = $this->cfield_mgr->get_linked_cfields_at_execution(
      $tproject_id,
      self::ENABLED,
      $show_on_execution,
      'testplan',
  ...
  function getEstimatedExecutionTimeFromCF($id, $itemSet = null, $platformID = null)
  {
    ...
    if ($status_ok) {
      ...
      if (is_null($itemSet)) {
        ...
        $rs = $this->db->fetchColumnsIntoMap(
          $sql4tplantcv,
          'platform_id',
          'tcversion_id',
          database::CUMULATIVE
  ...
  */
  function get_execution_time($context, $execIDSet = null)
  {
    ...
    if (is_null($cf_info)) {
      ...
      return $this->getExecutionTimeFromCF(
        $context->tplan_id,
        $execIDSet,
        $context->platform_id
      );
  ...
  */
  ...
  function exportTestPlanDataToXML($id, $context, $optExport = array())
  {
    ...
    if (isset($context['build_id']) && $context['build_id'] > 0) {
      ...
      $item_info['target_build'] = exportDataToXML(
        $mm,
        $xml_root,
        $xml_template,
        $xml_mapping,
  ...
  function getSkeleton($id, $tprojectID, $filters = null, $options = null)
  {
    ...
    //
    if (
      !is_null($my['filters']['testcase_name']) || !is_null($my['filters']['testcase_id']) ||
  ...
  /**
   * Optimized version of getSkeleton that builds the tree bottom-up from a set of leaf IDs (test cases).
   * This avoids the N+1 query problem present in the recursive approach.
   *
   * @param array $leaf_ids Array of node IDs (Test Cases) to build the tree from
   * @param int $root_id The root node ID (Test Project ID) to stop at
   * @return array array($rootNode, $tcaseSet) matching getSkeleton's return format
   */
  function getTreeFromLeaves($leaf_ids, $root_id)
  {
    if (empty($leaf_ids)) {
      return array(null, array('nindex' => array()));
    }

    $tcaseSet = array('nindex' => array());
    foreach ($leaf_ids as $lid) {
      $tcaseSet['nindex'][] = $lid;
    }

    $all_node_ids = array_flip($leaf_ids);
    $current_level_ids = $leaf_ids;

    // Bottom-up fetch of ancestors
    while (!empty($current_level_ids)) {
      $ids_str = implode(',', $current_level_ids);
      $sql = "SELECT DISTINCT parent_id FROM {$this->tables['nodes_hierarchy']} " .
        "WHERE id IN ($ids_str) AND parent_id IS NOT NULL";

      // Use fetchColumns if available or fetchRowsIntoMap
      $parents_rows = $this->db->fetchRowsIntoMap($sql, 'parent_id');
      $parents = array_keys($parents_rows);

      $next_level_ids = array();
      if ($parents) {
        foreach ($parents as $pid) {
          // stop at root (or 0) and avoid loops/duplicates
          if ($pid > 0 && !isset($all_node_ids[$pid])) {
            $all_node_ids[$pid] = true;
            $next_level_ids[] = $pid;
          }
        }
      }
    ...
    // Fetch details
    $all_ids_str = implode(',', array_keys($all_node_ids));

    // Need valid node_order for sorting.
    $sql = "SELECT id, parent_id, name, node_type_id, node_order " .
      "FROM {$this->tables['nodes_hierarchy']} " .
      "WHERE id IN ($all_ids_str) " .
      "ORDER BY node_order, id";

    $rows = $this->db->fetchRowsIntoMap($sql, 'id');
    if (!$rows)
      return array(null, $tcaseSet);
    ...
    foreach ($rows as $id => $row) {
      $node = $row;
      // Add type info (from tree_manager)
      $node['node_type'] = $this->tree_manager->node_types[$row['node_type_id']];
      $node['node_table'] = $this->tree_manager->node_tables_by['id'][$row['node_type_id']];
      $node['childNodes'] = null;

      if ($node['node_table'] == 'testcases') {
        $node['leaf'] = true;
        $node['external_id'] = ''; // Placeholder
      }
      $node_map[$id] = $node;
    }
    ...
    foreach ($node_map as $id => &$node) {
      ...
      // Basic self-cycle check
      if ($id == $pid) {
        // CYCLE DETECTED: Node $id has parent_id $pid (self)
        continue;
      }

      // Robust checks for multi-level cycles (A->B->A)
      $is_cycle = false;
      $curr_pid = $pid;
      $depth = 0;
      // Traverse up to 100 levels to find if 'id' is an ancestor of 'pid'
      while ($curr_pid > 0 && isset($node_map[$curr_pid]) && $depth < 100) {
        if ($curr_pid == $id) {
          $is_cycle = true;
          break;
        }
        $curr_pid = $node_map[$curr_pid]['parent_id'];
        $depth++;
      }

      if ($is_cycle) {
        // CYCLE DETECTED: Node $id is an ancestor of its parent $pid (Back edge)
        continue; // Do not link this node
      }

      if ($id == $root_id) {
        $root = &$node;
      } elseif (isset($node_map[$pid])) {
        if ($node_map[$pid]['childNodes'] === null) {
          $node_map[$pid]['childNodes'] = array();
        }
        // Add reference to child node
        $node_map[$pid]['childNodes'][] = &$node;
      }
    }
    unset($node); // Break reference

    // If root wasn't found in the map (e.g. partial tree?), we might have issues.
    // But usually root_id is the test project ID, which should be the top-most parent fetched.
    // If the loop didn't reach root_id (e.g. disconnected or filtered out?), then we return null or best effort.

    return array($root, $tcaseSet);
  }
  ...
  function getHitsSameStatusFullALOP($id, $statusSet, $buildSet = null, $opt = null)
  {
    ...
    list($safe_id, $buildsCfg, $sqlLEX) = $this->helperGetHits(
      $id,
      null,
      $buildSet,
  ...
  function getHitsStatusSetOnLatestExecOnPlatform($id, $platformID, $statusSet, $buildSet = null)
  {
    ...
    list($safe_id, $buildsCfg, $sqlLEBP) = $this->helperGetHits(
      $id,
      $platformID,
      $buildSet,
  ...
  function initGetLinkedForTree($tplanID, $filtersCfg, $optionsCfg)
  {
    ...
    if (
      isset($ic['options']['assigned_on_build']) &&
  ...
  function getLinkedFeatures($id, $filters = null, $options = null)
  {
    ...
    $items = $this->db->fetchMapRowsIntoMap(
      $sql,
      $my['options']['accessKey'][0],
